...
class GameManager(object):
    ...
    def setup(self):
        ...
        print("Double Down and Split bets can be less than or equal to original bet.")
    ...
    def ask_to_quit(self):
        ...
        while user_input != 'C' and user_input != 'Q':
            user_input = input("Invalid input, enter 'C' to coninue or 'Q' to quit.\n")
        if user_input == 'Q':
            self.quit = True
            return True
        return False
    ...
    def get_bet(self):
        ...
        while True:
            try:
                ...
                print("Valid bets are greater than 0 and less than {} chips.".format(self.user.get_chips()))
            user_input = input("Enter integer ammount to bet.\n")
    ...
    def play_hand(self, bet):
        ...
        if dealer_bj and user_bj:
            ...
            self.display_table(self.user.get_hand(), bet, hide_dealer=False)
        ...
        if choice == 'H':
            while choice != 'S':
                ...
                while choice != 'H' and choice != 'S':
                    choice = input("Invalid input, please select 'H' to hit, 'S' to stand.\n")
        elif choice == 'D':
            double_down = self.get_bet()
            while double_down > bet or double_down > self.user.get_chips() - bet:
                if double_down > self.user.get_chips() - bet:
                    print("Double down bet can't be greater than your remiaining chips.")
                else:
                    print("Double down bet must be less than or equal to original bet.")
                double_down = self.get_bet()
            bet += double_down
            self.user.add_cards(self.dealer.deal())
            if check_for_bust(self.user.get_hand()):
            ...
            while split_bet > bet or split_bet > self.user.get_chips() - bet:
                if split_bet > self.user.get_chips() - bet:
                    print("Split bet can't be greater than your remiaining chips.")
                else:
                    print("Split bet must be less than or equal to original bet.")
                split_bet = self.get_bet()
            hand_1 = Stack()
            hand_2 = Stack()
            ...
            if get_hand_total(self.dealer.get_hand()) < 17:
                self.display_table(hand_1, bet)
                self.display_table(hand_2, split_bet)
            else:
                self.display_table(hand_1, bet, hide_dealer=False)
                self.display_table(hand_2, split_bet, hide_dealer=False)
    ...
    def display_table(self, hand, bet, hide_dealer=True):
        ...
        for card in self.dealer.get_hand():
            ...
            hide_dealer = False
        for card in hand:
            user_cards.append(self.print_card(card))
        difference = self.dealer.get_hand().size - hand.size
        if difference < 0:
            for i in range(difference * -1):
                dealer_cards.append(self.print_card(place_holder=True))
        elif difference > 0:
            for i in range(difference):
                user_cards.append(self.print_card(place_holder=True))
        for i in range(max(len(dealer_cards), len(user_cards))):
            for j in range(7):
                print("{} | {}".format(dealer_cards[i][j], user_cards[i][j]))
         
    def print_card(self,card=None, hide_dealer=False, place_holder=False):
        lines = []
        if place_holder:
            lines.append('               ')
            lines.append('               ')
    ...
    def user_choice(self, bet):
        ...
        while choice != 'H' and choice != 'S' and choice != 'D' and choice != 'P':
            choice = input("Please select 'H' to hit, 'S' to stand, 'D' to double-down, or 'P' to split.\n")
